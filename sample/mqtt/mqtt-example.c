...
void event_handle(void *pcontext, void *pclient, iotx_mqtt_event_msg_pt msg)
{
    ...
    switch (msg->event_type) {
        ...
        case IOTX_MQTT_EVENT_DISCONNECT:
            EXAMPLE_TRACE("MQTT disconnect断开.");
            break;

        case IOTX_MQTT_EVENT_RECONNECT:
            EXAMPLE_TRACE("MQTT reconnect重连.");
            break;

        case IOTX_MQTT_EVENT_SUBCRIBE_SUCCESS:
            EXAMPLE_TRACE("subscribe订阅 success, packet-id=%u", (unsigned int)packet_id);
            break;

        case IOTX_MQTT_EVENT_SUBCRIBE_TIMEOUT:
            EXAMPLE_TRACE("subscribe wait等待 ack timeout, packet-id=%u", (unsigned int)packet_id);
            break;
        ...
        case IOTX_MQTT_EVENT_PUBLISH_RECVEIVED:
            EXAMPLE_TRACE("topic message arrived到达 but without any related handle: topic=%.*s, topic_msg=%.*s",
...
static void _demo_message_arrive(void *pcontext, void *pclient, iotx_mqtt_event_msg_pt msg)
{
    ...
    EXAMPLE_TRACE("----");
    EXAMPLE_TRACE("Topic主题到达: '%.*s' (Length: %d)",
...
int mqtt_client(void)
{
    ...
    if (NULL == pclient) {
        EXAMPLE_TRACE("MQTT construct结构 failed");
        rc = -1;
        goto do_exit;
    }
    ...
    strcpy(msg_pub, "message: hello! start开始!");
	...
	while(scanf("%s",input_value) != 1) printf("must input one string:");
        /* Generate topic message */
        cnt++;
        msg_len = snprintf(msg_pub, sizeof(msg_pub), "{\"attr_name\":\"temperature\", \"attr_value\":\"%d\",, \"input_value\":\"%s\"}", cnt,input_value);
        if (msg_len < 0) {
        ...
        EXAMPLE_TRACE("packet-id=%u, publish topic msg消息='0x%02x%02x%02x%02x'...",
                      (uint32_t)rc,
...
int mqtt_client_secure()
{
    ...
    /* Initialize topic information */
    memset(&topic_msg, 0x0, sizeof(iotx_mqtt_topic_info_t));
    strcpy(msg_pub, "message: hello! start!又开始");
	...
	while(scanf("%s",input_value) != 1) printf("must input one string:");
        ...
        EXAMPLE_TRACE("packet-id=%u, publish topic 又消息msg='0x%02x%02x%02x%02x'...",
                      (uint32_t)rc,
                      msg_pub[0], msg_pub[1], msg_pub[2], msg_pub[3]
                     );
